...
BENCHMARK_QUERIES = [
    ...
    (
        "q01_dutoan_chatgpt",
        "Dự toán chi mua bản quyền ChatGPT Teams 2026 là bao nhiêu?",
        "427/QĐ-TNVN",
    ),
    (
        "q02_dutoan_phanmem_ai",
        "Kinh phí thuê tài khoản NotebookLM Audio Overviews năm 2026",
        "427/QĐ-TNVN",
    ),
    # 2. Nhân sự / Chấm dứt HĐ
    (
        "q03_nhansu_cham_dut_hd",
        "Chấm dứt hợp đồng làm việc của bà Nguyễn Thị Kim Lan",
        "80-QĐ/TNVN",
    ),
    (
        "q04_nhansu_ly_do_thoi_viec",
        "Thời điểm thôi hưởng lương của viên chức Nguyễn Thị Kim Lan",
        "80-QĐ/TNVN",
    ),
    # 3. Khen thưởng
    (
        "q05_khenthuong_tapthe",
        "Khen thưởng danh hiệu Tập thể lao động xuất sắc năm 2025 gồm những đơn vị nào?",
        "109-QĐ/TNVN",
    ),
    (
        "q06_khenthuong_coquan_tnvn",
        "Ban Tổ chức cán bộ và Hợp tác quốc tế được tặng Bằng khen gì năm 2025?",
        "1119/QĐ-TNVN",
    ),
    # 4. Đào tạo
    (
        "q07_daotao_ai_toasoan",
        "Tập huấn ứng dụng AI trong tòa soạn do chuyên gia nào giảng dạy?",
        "87/QĐ-TNVN",
    ),
    (
        "q08_daotao_ngach_chuyenvien",
        "Kế hoạch bồi dưỡng ngạch chuyên viên và tương đương năm 2026",
        "50/QĐ-TNVN",
    ),
    # 5. Kỹ thuật / Phát sóng
    (
        "q09_phatsong_quangninh_cot5",
        "Dự án phát sóng FM tại trạm Cột 5 Quảng Ninh nghiệm thu năm nào?",
        "2244/QĐ-TNVN",
    ),
    (
        "q10_phatsong_may_fm",
        "Bảo dưỡng máy phát sóng FM trạm Cột 5 Quảng Ninh năm 2026",
        "72/QĐ-TNVN",
    ),
    # 6. Chỉ thị
    (
        "q11_chithi_evfta",
        "Chỉ thị về công tác thông tin tuyên truyền Hiệp định EVFTA",
        "1838/CT-TNVN",
    ),
    # 7. Multi-document & Signer queries (Truy vấn đa văn bản & Người ký)
    (
        "q12_multi_doc_signer_vhq",
        "Phó Tổng Giám đốc Vũ Hải Quang ký những văn bản nào?",
        "Vũ Hải Quang",
    ),
    ("q13_multi_doc_signer_dts", "Tổng Giám đốc Đỗ Tiến Sỹ ký những văn bản nào?", "Đỗ Tiến Sỹ"),
    (
        "q14_multi_doc_signer_nmh",
        "Phó Tổng Giám đốc Ngô Minh Hiển ký những văn bản nào?",
        "Ngô Minh Hiển",
    ),
    (
        "q15_multi_doc_signer_pmh",
        "Phó Tổng Giám đốc Phạm Mạnh Hùng ký những văn bản nào?",
        "Phạm Mạnh Hùng",
    ),
    (
        "q16_multi_doc_topic_ai",
        "Các quyết định về ứng dụng trí tuệ nhân tạo và ChatGPT tại VOV",
        "Trí tuệ nhân tạo",
    ),
    (
        "q17_multi_doc_phatsong_quangninh",
        "Các quyết định liên quan đến trạm phát sóng Cột 5 Quảng Ninh",
        "Cột 5 Quảng Ninh",
    ),
    (
        "q18_multi_doc_thamnien_2026",
        "Phụ cấp thâm niên vượt khung cho cán bộ viên chức năm 2026",
        "862/QĐ-TNVN",
    ),
]
...
async def main() -> None:
    ...
    async with session_factory() as session:
        doc_count = (
            await session.execute(text("SELECT count(*) FROM documents WHERE is_active = true"))
        ).scalar()
    ...
    print(
        f"Database Corpus: {doc_count} Active Documents | {chunk_count} Chunks | {vec_count} pgvector Embeddings"
    )
    print("=" * 95 + "\n")
    ...
    print(
        f"{'No':2s} | {'Query Description / Text':45s} | {'Expected Target':18s} | {'Hybrid Top-1 Match':35s} | {'Latency':7s}"
    )
    print("-" * 115)

    for idx, (_qid, query_text, target) in enumerate(BENCHMARK_QUERIES, start=1):
        q = RetrievalQuery(
        ...
        print(
            f"{idx:02d} | {query_text[:45]:45s} | {target:18s} | {top1_snippet:35s} | {lat_ms:5.1f}ms"
        )

    print("-" * 115)
    print(f"Total Hybrid Queries: {len(BENCHMARK_QUERIES)}")
    print(
        f"Average Live Hybrid Latency: {total_hybrid_time / len(BENCHMARK_QUERIES):.1f}ms (including live query embedding + pgvector HNSW + FTS + RRF)"
    )
    print("=" * 115 + "\n")
