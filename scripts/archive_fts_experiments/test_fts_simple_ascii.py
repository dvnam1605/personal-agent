...
async def main():
    ...
    try:
        print("=== Testing FTS Accuracy with OCR Corpus (ASCII output) ===")
            ...
            try:
                ...
                if len(rows) == 0:
                    ...
                    for r in rows:
                        ...
                        if len(snippet) > 150:
                            snippet = snippet[:150] + '...'
                        # Print file name and snippet (may contain non-ASCII, but we'll try)
                        # We'll encode to ASCII with errors ignored for safety
                        safe_snippet = snippet.encode('ascii', errors='ignore').decode('ascii')
                        safe_filename = r['file_name'].encode('ascii', errors='ignore').decode('ascii')
                        print(f'    [{safe_filename}] {safe_snippet} (rank={r["rank"]:.4f})')
            except Exception as e:
        ...
        for r in rows:
            safe_filename = r['file_name'].encode('ascii', errors='ignore').decode('ascii')
            print(f'  ID: {r["id"]}, File: {safe_filename}')
